...
## ⭐ 빌드 후 골드 비교 보고 의무 (v4.1 확정)

### 핵심 규칙 ⭐

```
1단계(11b 전체 PDF → HWPX 완성)가 끝날 때까지,
학교 하나 v5 빌드 완료 시 반드시 골드와 비교하고 상세 보고한다.
```

**절대 생략 불가.** 빌드 → 보고 → 다음 학교 순서.
...
[구조]
  문제 수:   v5=N개 (선다형 A + 서술형 B) / 골드=N개 → ✓ 일치 / ✗ 불일치
  선택지:    v5=N개 / 골드=N개 → ✓ 일치 / ✗ 불일치

[수식]
  v5=N개 / 골드=N개 → 차이 ±N개
  (골드 대비 -20% 이상 차이 시 원인 분석 필요)

[문제별 이슈]
  N번: [이슈 내용] (예: 선택지 누락, 수식 손상, 문제 텍스트 오류 등)
  ...
  이슈 없으면: "전 문제 선택지·배점 일치, 특이사항 없음"

[종합 판정]
  ✓ PASS / ✗ 추가 수정 필요
  추가 수정 필요 시: 원인 + 조치 방법 제시
```

### 비교 방법

```powershell
# 골드 manifest 생성 (이미 있으면 스킵)
python scripts/analyze_gold_hwpx.py [학교명]

# v5 vs 골드 비교 (수동 Python 분석)
# - gold_manifest/[학교].json 로드
# - v5 hwpx XML에서 선택지 마커 수, hp:script 수 추출
# - 문제별 선택지 수 비교
```
