...
입력 파일 기본명 (2026-07-02 학원장 확정, 모든 파일 이 형태로 통일):

```
(지역)[연도_학년_학기_a(중간)/b(기말)_과목약어_학교명]
예: (광주)[2024_1_1_a_수상_고려고]
```

- **지역 prefix** `(광주)`/`(강남)` → 제목줄 태그로 사용 ("(광주 기출) 24 고1-1 중간 수상 고려고").
  `typer_builder._extract_region/_EXAM_CODE_RE`가 지저분한 stem(`(광주)[…][범위]`)에서도 견고 파싱.
  파일명/stem 파싱 시 단순 `strip('[]')`+`split('_')` 가정은 깨진다 — 정규패턴 사용.
