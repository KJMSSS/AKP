...
[그림·표 — 내용을 발문에 옮겨 적지 마라. 자주 틀리니 특히 주의]
- 문제에 딸린 그림/그래프/도형이 있던 자리는 stem 에 "[그림]" 토큰 '하나만' 넣어라.
- 문제에 딸린 표(도수분포표·확률분포표·자료표 등 격자)가 있던 자리는 stem 에 "[표]" 토큰 '하나만' 넣어라.
- ⚠️ 표 안의 셀 내용(글자·숫자·항목명)을 stem 에 텍스트로 옮겨 적지 마라. 그래프의 축 라벨·
  눈금 숫자·곡선 위 좌표값도 stem 에 옮겨 적지 마라. 표/그림은 이미지로 따로 삽입되므로,
  내용을 옮겨 적으면 시험지에 같은 내용이 두 번 찍힌다.
- 표/그림 '밖'에 인쇄된 일반 문장(도입·질문·조건 서술)은 정상적으로 stem 에 포함한다.
- 각 문제마다 has_figure(그림/그래프/도형 존재)와 has_table(표 존재)을 true/false 로 정확히 표기하라.
  검수 화면이 이 값을 보고 "그림/표를 크롭해 넣으세요" 경고를 띄운다.
...
형식:
{"problems":[{"number":"1","points":"3.4점(없으면 빈문자열)","stem":"발문 전체(수식 \\(..\\))","choices":["보기1","보기2",...],"has_figure":false,"has_table":false}]}
...
_FIG_PROMPT = """이 이미지는 한국 시험지(수학·과학탐구 등) 한 페이지(가로 {W}px, 세로 {H}px)다.
'시험 문제에 속한 그림/그래프/도표/모식도'를 모두 찾아라:
- 수학: 함수 그래프, 평면도형(삼각형·원 등), 입체도형(정사면체·정육면체·원기둥 등),
  좌표평면/좌표공간, 기하 다이어그램, 막대/꺾은선 등 통계 도표.
- 과학(생물·화학·물리·지구): 세포·기관·생물체 모식도, 화살표로 연결된 과정도(물질대사·
  신경 전달·순환 등), 트리/분류 구조도, 실험 장치도, 해부도·구조도, 그래프/그림표.

**그림에 딸린 작은 설명 상자·라벨·범례·기호 정의도 그림의 일부이니 함께 감싸라**
(예: 그림 옆의 '특징(A~C) ○㉠ ○…' 네모 상자, '(가)/(나)' 라벨, '→: 있음' 범례).
같은 문제의 그림 요소가 여러 개 흩어져 있으면 그 전체를 넉넉히 감싸는 하나의 bbox 로 잡아라
(도입 문장·질문 문장·<보기> 상자·①~⑤ 선택지는 제외하고, 그 사이의 '그림 영역'만).

다음은 **절대 포함하지 마라**(오검출이 매우 잦으니 특히 주의):
- **시험지 상단 서식**: '성명'·'수험 번호' 입력란(네모칸 줄), '제 N 교시' 배너, 과목명
  헤더, 페이지 번호 — 이건 문제 그림이 절대 아니다.
- 로고·배너·광고·캐릭터·마스코트, "정답 및 해설"·"선생님이 직접 풀어주신" 등 해설지/출판사
  안내 그림, 표지·워터마크·열쇠/리본 같은 장식 이미지
- 수식, 일반 텍스트(도입·지문·질문), <보기> 상자(ㄱㄴㄷ — 별도 처리), 보기 ①~⑤,
  격자형 표(행/열 있는 도수분포표·측정표 — 별도 처리), 학생 손글씨/낙서, 도장·직인,
  머리말·꼬리말
그림/모식도(도형·화살표·구조가 '그려진' 것)이면 포함, 순수 글자·문장이면 제외.

각 그림이 '어느 문제에 속하는지' 문제 번호도 판단하라(그림 주변/위의 문항 번호).
JSON 으로만 출력: {"figures":[{"label":"간단설명","bbox":[x0,y0,x1,y1],"problem":"문제번호(모르면 빈문자열)"}]}
- **bbox 정확도가 매우 중요**: 그림 요소(곡선·도형·좌표축·화살표·구조·점·라벨·딸린 설명상자)를
  빠짐없이 '딱 맞게' 감싸라. 학생 손글씨 풀이가 그림 위/옆에 겹쳐 있어도 인쇄된 그림 본체를
  기준으로 감싸되, 여백의 손글씨 계산식까지 넓게 잡지 마라. 그림이 작거나 흐릿해도 그 그림
  영역을 정확히 지정하라. bbox 안에 그림이 없고 텍스트/여백만 있으면 잘못된 것이다.
- bbox 는 픽셀 정수 좌표(좌상단 x0,y0 / 우하단 x1,y1).
- 그림이 없으면 {"figures":[]}. JSON 외 출력 금지."""
...
class Region:
    ...
    kind: str = ""       # 표 감지에서만: "grid"(격자 표) | "box"(테두리 상자)
...
_TABLE_DETECT_PROMPT = """이 이미지는 시험지 한 페이지(가로 {W}px, 세로 {H}px)다.
'시험 문제에 딸린' 다음 두 종류의 인쇄 영역을 모두 찾아라:
1) kind="grid" — 격자 표(행/열 선이 있는 것: 도수분포표·확률분포표·자료표·참거짓표 등)
2) kind="box"  — 테두리 상자(수식·조건·유도 과정·자료가 사각 테두리 '한 칸' 안에 인쇄된
   조건 상자/과정 상자. 내부에 행/열 구분선 없음)

다음은 **절대 포함하지 마라**:
- 시험지 머리말/양식: 시험명·날짜·교시·학년 제목, 반·번·이름 칸, **배점표**(선다형·서술형
  문항수×점수 표), 유의사항·안내문, 저작권 문구, 쪽번호. (배점표는 격자여도 문제의 표가 아니다.)
- <보기> 상자(ㄱ. ㄴ. ㄷ. … 항목 나열) — 별도 공정에서 처리하므로 제외.
- 수식 속 빈칸 네모(□)·답 기입용 작은 네모, 보기①~⑤, 일반 텍스트·수식,
  그림/그래프/도형, 학생 손글씨·연필 낙서, 2단 레이아웃의 단 경계 세로줄,
  문제 여러 개를 두르는 레이아웃 박스.
JSON 으로만 출력:
{"tables":[{"label":"설명","kind":"grid|box","bbox":[x0,y0,x1,y1],"problem":"문제번호(모르면 빈문자열)"}]}
- bbox 는 픽셀 정수 좌표(좌상단 x0,y0 / 우하단 x1,y1). 테두리 선까지 포함해 잡아라.
- 해당 영역이 없으면 {"tables":[]}. JSON 외 출력 금지."""
...
# 테두리 상자(조건·과정 상자)는 내부 구분선이 없는 '한 칸'이므로 grid 가 아니라
# 줄 목록으로 읽는다 → 호출부가 1×1 표(다중 문단 셀)로 조립해 원본 상자를 재현.
_BOX_READ_PROMPT = """이 이미지는 시험 문제에 딸린 '테두리 상자'(조건·과정·자료 상자)를 잘라낸 것이다.
상자 안에 '인쇄된' 내용을 줄 단위로 정확히 읽어라.

규칙:
- 학생 손글씨·연필 낙서·밑줄·동그라미는 **무시**하고, 인쇄된 원래 내용만 읽는다.
- 크롭에 상자 '테두리 밖' 주변 글자가 일부 걸려 있으면 무시하고 **테두리 안** 내용만 읽는다.
- 한글은 정확히(절대 한자/일본어로 바꾸지 말 것). 숫자·기호도 정확히.
- 수식은 LaTeX 를 \\( ... \\) 로 감싸 표기. 인쇄된 줄 순서를 유지하고 빈 줄은 넣지 않는다.
JSON 으로만 출력: {"lines":["첫 줄", "둘째 줄", ...]}
읽을 수 없으면 {"lines":[]}. JSON 외 출력 금지."""


def read_box_lines(image_path: str, *, model: str | None = None) -> list[str]:
    """크롭된 테두리 상자 이미지 -> 인쇄된 줄 목록(수식은 \\(..\\) LaTeX)."""
    content = [_encode_image(image_path), {"type": "text", "text": _BOX_READ_PROMPT}]
    out = _call(content, max_tokens=2000, model=model)
    data = _extract_json(out)
    lines = data.get("lines", []) if isinstance(data, dict) else data
    return [str(ln).strip() for ln in (lines or []) if str(ln).strip()]
