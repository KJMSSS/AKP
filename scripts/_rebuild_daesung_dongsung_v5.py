...
def build_one(source, preprocess_fn):
    ...
    segments = normalize_choices(segments, log_stem=f"{stem}_v5fix2")

    print("\n=== rebuild ===")
    md_rebuilt = rebuild_markdown(header, segments)

    print("\n=== LLM 후처리 ===")
    md_llm, llm_meta = postprocess_markdown(md_rebuilt, log_stem=f"{stem}_v5fix2")
