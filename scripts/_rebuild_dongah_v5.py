...
print("\n=== 선택지 정규화 ===")
segments = normalize_choices(segments, log_stem="동아여고_v5")

print("\n=== rebuild ===")
md_rebuilt = rebuild_markdown(header, segments)

print("\n=== LLM 후처리 ===")
md_llm, llm_meta = postprocess_markdown(md_rebuilt, log_stem="동아여고_v5")
if llm_meta.get("skipped"):
    print(f"  스킵: {llm_meta.get('reason')}")
else:
    cost = llm_meta.get("cost_usd", 0)
    print(f"  완료 (${cost:.4f})")
