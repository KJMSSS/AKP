...
- 함수·확통: 합성함수 $f \\circ g$, 역함수 $f^{-1}$, 도함수 $f'(x)$, 순열 ${}_{n}P_{r}$, 조합 ${}_{n}C_{r}$, 경우나누기 $\\begin{cases} ... \\end{cases}$
...
- ❌ a_n  →  ✅ a_{n}   (첨자 중괄호 누락)
- ❌ 전체집합 의 (집합 기호 누락)  →  ✅ 전체집합 $U$의
- ❌ 집합 A B  →  ✅ 집합 $A \\cap B$ / $A \\cup B$  (집합 연산 기호 누락)

[집합 기호 — 특히 자주 누락되니 주의]
전체집합 $U$, 공집합 $\\varnothing$, 원소 $\\in \\notin$, 부분집합 $\\subset \\supset$,
합집합 $\\cup$, 교집합 $\\cap$, 차집합 $A - B$, 여집합 $A^{c}$ 를 절대 빠뜨리지 말 것.
'전체집합 U의 부분집합', '집합 $A \\cap B$' 같은 표현에서 기호를 누락하면 안 된다.
단, 원본에서 정말 읽을 수 없으면 추측하지 말고 그대로 둔다(환각 금지).
...
- 함수·확통: 합성 f \circ g, 역함수 f^{-1}, 도함수 f'(x), 순열 {}_{n}P_{r}, 조합 {}_{n}C_{r}, 경우나누기 \begin{cases} ... \end{cases}
- 삼각함수: \sin, \cos, \tan, \log (백슬래시 필수)
- 그리스 문자: \alpha, \beta, \pi, \theta 등
- 부등호: \leq, \geq, \neq
- 집합 기호: \in, \notin, \subset, \supset, \cup, \cap, 전체집합 U, 공집합 \varnothing, 여집합 A^{c} — 절대 누락 금지
- 무한대: \infty
- 행렬·cases 환경: \begin{cases} ... \end{cases}
- 괄호 크기 자동 조절: \left( ... \right), \left[ ... \right]

[집합 기호 누락 주의] '전체집합 U의 부분집합', '집합 A∩B' 같은 표현에서
전체집합 U·합집합·교집합·부분집합 기호를 빠뜨리지 말 것. 단 정말 안 읽히면 추측 금지.
